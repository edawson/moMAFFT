				...
				case( 'L' ):
					...
					else
					{
						if( store_localhom )
							...
							pscore = L__align11_noalign( mseq1, mseq2 );
					}
					break;
				case( 'Y' ):
...
#endif
			if( pscore > 0.0 && ( nadd == 0 || ( alg != 'Y' && alg != 'r' ) || ( i < njob-nadd && njob-nadd <= j ) ) )
			{
				if( !store_localhom )
					;
				else if( alg == 'R' )
...
static void pairalign( char **name, int *nlen, char **seq, char **aseq, char **mseq1, char **mseq2, double *effarr, int alloclen, Lastresx **lastresx )
{
	int i, j, ilim, jst, jj;
	int clus1, clus2;
	int off1, off2;
						...
						case( 'L' ):
							...
							else
							{
								if( store_localhom )
									...
									pscore = L__align11_noalign( mseq1, mseq2 ); // all pair
							}
							break;
						case( 'Y' ):
							if( nadd == 0 || ( i < njob-nadd && njob-nadd <= j ) ) // new sequence vs exiting sequence nomi keisan
...
//	writePre( njob, name, nlen, seq, 0 );
	...
	pairalign( name, nlen, bseq, aseq, mseq1, mseq2, eff, alloclen, lastresx );
